...
function play3r(div, baseNote) {
	...
	reverb(len, div, p2, matrix[1], 1, 0.5, playNoteInScale, [1, scale,  1 + baseNote,      90,  pressLen]);
	reverb(len, div, p3, matrix[2], 1, 0.5, playNoteInScale, [2, scale,  2 + baseNote,      90,  pressLen]);
	reverb(len, div, p4, matrix[3], 1, 0.5, playNoteInScale, [3, scale,  3 + baseNote,      90,  pressLen]);

	reverb(len, div, p1, matrix[4], 1, 0.5, playNoteInScale, [4, scale,  4 +      baseNote,  110,  pressLen]);
	reverb(len, div, p2, matrix[5], 1, 0.5, playNoteInScale, [5, scale,  4 + 5 +  baseNote,  110,  pressLen]);
	reverb(len, div, p3, matrix[6], 1, 0.5, playNoteInScale, [6, scale,  2 + 10 + baseNote,  110,  pressLen]);
	reverb(len, div, p4, matrix[7], 1, 0.5, playNoteInScale, [7, scale,  2 + 10 + baseNote,  110,  pressLen]);
}
